...
public class McpController {
    ...
    public List<McpPrompt> listPrompts() {
        return List.of(
            new McpPrompt(
                "daily_checkin",
                "A starter for daily reflection",
                "How are you feeling today? Would you like to record a diary entry?"
            ),
            new McpPrompt(
                "distortion_check",
                "Help identifying negative thoughts",
                "I noticed some strong language in your last entry. Shall we look for cognitive distortions together?"
            )
        );
    }
    ...
    public Object callTool(@RequestBody Map<String, Object> request) {
        ...
        if ("thought_analyzer".equals(name) && args != null && args.get("text") != null) {
            return aiService.analyzeThought(args.get("text").toString());
        }

        return Map.of("message", "Tool execution logic not found");
    }
}
